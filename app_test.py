...
def update_rank_and_division(rank, division, current_lp):
    ...
    # Handle LP that is enough to advance to the next division or rank
    while current_lp >= 100:
        ...
        if division_index == 3:
            ...
            if current_rank_index >= len(ranks):
                current_rank_index = len(ranks) - 1
                break  # Stop if it's the highest rank
        else:
            division_index += 1
    
    # Handle LP that is enough to drop to the previous division or rank
    while current_lp < 0:
        current_lp += 100
        if division_index == 0:
            ...
            if current_rank_index < 0:
                ...
                break  # Stop if it's the lowest rank
        else:
            division_index -= 1
    ...
    return rank, division, current_lp
...
def generate_leaderboard():
    ...
    # Get the total LP change from logs
    query = '''
    SELECT 
        p.player_name, 
        p.rank, 
    ...
    leaderboard = leaderboard[['player_name', 'LP Gained/Lost', 'current_lp', 'rank', 'division', 'Rank']].sort_values(by='Rank')
    return leaderboard
...
if page == "Register Player":
    ...
    rank = st.selectbox('Select Rank', ['Iron', 'Bronze', 'Silver', 'Gold', 'Platinum', 'Emerald', 'Diamond', 'Master', 'Grandmaster', 'Challenger'])
